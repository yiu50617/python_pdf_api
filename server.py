...
CLAIM_TYPES = {
    "parent_death": {"label": "死亡弔慰金", "code": "12", "amount": 20000},
...
def validate_input(data: dict) -> None:
    ...
    hospital_phone = data.get("hospitalPhone", "")
    account = data.get("accountNumber", "")
    if not re.fullmatch(r"\d{7}", member_number):
        raise ValueError("組合員番号は半角数字7桁で入力してください。")
    if phone and not re.fullmatch(r"\d{10,11}", phone):
        raise ValueError("日中連絡先はハイフン不要の半角数字10桁または11桁で入力してください。")
    if hospital_phone and not re.fullmatch(r"\d{10,11}", hospital_phone):
        raise ValueError("病院電話番号はハイフン不要の半角数字10桁または11桁で入力してください。")
...
def draw_claim(data: dict):
    ...
    def _draw(c: canvas.Canvas) -> None:
        ...
        draw_center(c, 410, 720, data.get("representativeName", ""), 9)

        draw_center(c, 95, 687, claim_year, 9)
        draw_center(c, 135, 687, claim_month, 9)
        draw_center(c, 165, 687, claim_day, 9)

        draw_center(c, 245, 520, data.get("memberFamilyName", ""), 8)
        draw_center(c, 325, 520, data.get("memberGivenName", ""), 8)
        draw_center(c, 235, 542, data.get("memberFamilyKana", ""), 6.5)
        draw_center(c, 325, 542, data.get("memberGivenKana", ""), 6.5)
        ...
        draw_center(c, 210, 185, data.get("bankNameBase", ""), 7.5)
        ...
        draw_center(c, 320, 185, data.get("branchNameBase", ""), 7.5)
        ...
        draw_center(c, 350, 161, data.get("accountHolderKana", ""), 7.5)
        draw_center(c, 350, 137, data.get("accountHolderName", ""), 8)
...
def draw_certificate(data: dict):
    ...
    def _draw(c: canvas.Canvas) -> None:
        draw_center(c, 210, 687, data.get("memberFamilyName", ""), 8)
        draw_center(c, 325, 687, data.get("memberGivenName", ""), 8)
        draw_center(c, 220, 710, data.get("memberFamilyKana", ""), 6.5)
        draw_center(c, 325, 710, data.get("memberGivenKana", ""), 6.5)
        ...
        if claim == "parent_death":
            draw_center(c, 200, 610, data.get("targetFamilyName", ""), 8)
            draw_center(c, 295, 610, data.get("targetGivenName", ""), 8)
            draw_center(c, 200, 627, data.get("targetFamilyKana", ""), 6.5)
            draw_center(c, 295, 627, data.get("targetGivenKana", ""), 6.5)
            draw_check(c, 155, 560)
            ...
            if data.get("targetGender") == "male":
                draw_check(c, 475, 608)
            elif data.get("targetGender") == "female":
                draw_check(c, 512, 608)
            draw_center(c, 467, 557, death[0], 8)
            draw_center(c, 497, 557, death[1], 8)
            draw_center(c, 523, 557, death[2], 8)
        elif claim == "sickness":
            draw_center(c, 360, 537, data.get("sicknessName", ""), 8)
            for index, y in [(1, 513), (2, 488), (3, 466)]:
                start = normalize_date(data.get(f"leavePeriod{index}Start", ""))
                end = normalize_date(data.get(f"leavePeriod{index}End", ""))
                if not any(start) and not any(end):
                    start, end = normalize_date_range(data.get(f"leavePeriod{index}", ""))
            ...
            draw_center(c, 290, 442, data.get("hospitalName", ""), 8)
            draw_center(c, 495, 442, data.get("hospitalPhone", ""), 8)
        elif claim == "marriage":
            ...
            draw_center(c, 200, 391, data.get("spouseFamilyName", ""), 8)
            draw_center(c, 295, 391, data.get("spouseGivenName", ""), 8)
            draw_center(c, 200, 412, data.get("spouseFamilyKana", ""), 6.5)
            draw_center(c, 295, 412, data.get("spouseGivenKana", ""), 6.5)
            draw_check(c, 175, 350)
            ...
            draw_center(c, 450, 352, data.get("marriageOffice", ""), 8)
        elif claim == "birth":
            child_birth = normalize_date(data.get("childBirthDate") or data.get("eventDate", ""))
            draw_center(c, 200, 302, data.get("childFamilyName", ""), 8)
            draw_center(c, 295, 302, data.get("childGivenName", ""), 8)
            draw_center(c, 200, 322, data.get("childFamilyKana", ""), 6.5)
            draw_center(c, 295, 322, data.get("childGivenKana", ""), 6.5)
            ...
            if data.get("overThreeYears", "yes") == "yes":
                draw_check(c, 253, 156)
            else:
                draw_check(c, 299, 156)
            ...
            if data.get("retirementReason") == "regular":
                draw_check(c, 219, 133)
            else:
                draw_check(c, 282, 133)
        ...
        draw_center(c, 425, 50, data.get("representativeName", ""), 8)
